<?php
	...
	if($total>1 && $pagina<=$Npaginas){
		...
		foreach($datos as $filas) {
			...
			<tr>
                <td>'.$contador.'</td>
                <td hidden><input name="id_control[]" value="'.$filas['id'].'" /></td>
                <td>'.$filas['estudiantes_cedula'].'</td>
                <td>'.$filas['estudiantes_nombres'].'</td>
                <td>'.$filas['estudiantes_apellidos'].'</td>
                <td><input name="teorica['.$filas['id'].']" value="'.$filas['evaluacion_teorica'].'" /></td>
                <td><input name="practica['.$filas['id'].']" value="'.$filas['evaluacion_practica'].'" /></td>
			';
			$contador++;
		}
